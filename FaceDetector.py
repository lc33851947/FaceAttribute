...
class mtcnn():
    def __init__(self,model_path,useGPU = True):
        # Load models.
        prototxt = [model_path + '/' + x + '.prototxt' for x in ['det1', 'det2', 'det3']]
        binary = [model_path + '/' + x + '.caffemodel' for x in ['det1', 'det2', 'det3']]
...
#        print('Image shape:', im.shape)
        ...
        scales = []
        maxFaceSize = min(image_height,image_width)
        faceSize = 40
        while faceSize <= maxFaceSize:
            currentScale = float(MIN_INPUT_SIZE) / faceSize
            scales.append(currentScale)
            faceSize  = faceSize /FACTOR    
        for scale in scales:
...
#        print('After PNet bboxes shape: ', bboxes.shape)
        ...
        self.RNet.blobs['data'].reshape(N,3,H,W)
        self.RNet.blobs['data'].data[...] = inputs
        outputs = self.RNet.forward()
    ...
    #    print('After RNet bboxes shape: ', bboxes.shape)
        ...
        N,C,H,W = inputs.shape
        self.ONet.blobs['data'].reshape(N,3,H,W)
        self.ONet.blobs['data'].data[...] = inputs
        outputs = self.ONet.forward()
...
if __name__ == '__main__':      
    detector = mtcnn("./model/mtcnn",useGPU = True)
    aa,bb = detector.detect("/home/liucheng/tmp/01.jpg")
